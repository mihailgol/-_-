  ...
  if (supportSendBtn) {
    supportSendBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        showToast("✉️ Обращение отправлено", `Заявка #${res.ticketId || 1} принята. Спасибо за обращение, в ближайшее время исправим проблему.`);
        inputs.forEach((i) => (i.value = ""));
      } catch {
        showToast("✉️ Обращение отправлено", "Спасибо за обращение, в ближайшее время исправим проблему.");
        inputs.forEach((i) => (i.value = ""));
      }
    });
  }
