...
- **Глобальный переключатель ЕГЭ/ОГЭ:** Переключатель в верхней строке (Header) является **главным (Master)** для всего приложения конкретного пользователя. Выбор экзамена определяет весь отображаемый контент (предметы, задания, тесты, теория). Наборы заданий и теории для ОГЭ и ЕГЭ строго разделены и не должны совпадать/дублироваться.
